  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, backgroundColor: C.white }]}>
      <View style={[styles.header, { borderBottomColor: C.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: C.black }]}>{t.notif_settings_title}</Text>
        <View style={{ width: 60 }} />
      </View>
      <View style={styles.content}>
        <View style={[styles.row, { borderBottomColor: C.borderLight }]}>
          <Text style={[styles.label, { color: C.black }]}>{t.notif_settings_likes}</Text>
          <Switch value={notifLikes} onValueChange={setNotifLikes} trackColor={{ true: C.primary }} />
        </View>
        <View style={[styles.row, { borderBottomColor: C.borderLight }]}>
          <Text style={[styles.label, { color: C.black }]}>{t.notif_settings_followers}</Text>
          <Switch value={notifFollowers} onValueChange={setNotifFollowers} trackColor={{ true: C.primary }} />
        </View>
        <View style={[styles.row, { borderBottomColor: C.borderLight }]}>
          <Text style={[styles.label, { color: C.black }]}>{t.notif_settings_comments}</Text>
          <Switch value={notifComments} onValueChange={setNotifComments} trackColor={{ true: C.primary }} />
        </View>
        <View style={[styles.row, { borderBottomColor: C.borderLight }]}>
          <Text style={[styles.label, { color: C.black }]}>{t.notif_settings_reminders}</Text>
